...
class BridgeBroker:
    """Process-internal routing library — no longer owns its own listen socket.

    The gateway (bridge_gateway) owns the northbound endpoint and calls
    `handle()` directly.  Worker lifecycle, session→worker routing, and
    idle GC are all preserved from the original hermes-studio broker.
    """
