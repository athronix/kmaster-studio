...
class LineReader:
    """Persistent line-oriented buffer for NDJSON frame reassembly.

    Accumulates bytes across recv() calls. Splits on ``\\n``. Retains
    partial trailing frames. Hard cap at MAX_LINE_BYTES to prevent
    memory exhaustion from runaway peers.
    """

    MAX_LINE_BYTES: int = 8 * 1024 * 1024  # 8 MB

    def __init__(self) -> None:
        self._buf = bytearray()

    def feed(self, chunk: bytes) -> list[bytes]:
        """Push a received chunk.  Returns zero or more complete lines."""
        ...
        while True:
            ...
            del self._buf[: idx + 1]
            # strip trailing carriage return for Windows compat
            if line.endswith(b"\r"):
                line = line[:-1]
            if len(line) > self.MAX_LINE_BYTES:
                raise RuntimeError(
                    f"FRAME_TOO_LARGE: line {len(line)} bytes exceeds limit {self.MAX_LINE_BYTES}"
                )
            lines.append(line)
        # safety: if buffer exceeds max without any newline, fail
        if len(self._buf) > self.MAX_LINE_BYTES:
            raise RuntimeError(
                f"FRAME_TOO_LARGE: buffer {len(self._buf)} bytes exceeds limit {self.MAX_LINE_BYTES}"
            )
        return lines

    def pending(self) -> int:
        """Return number of bytes waiting for a newline."""
...
class WorkerProcess:
    ...
    def start(self) -> None:
        with self._lock:
            ...
            args = [
                sys.executable,
                str(Path(__file__).with_name("kmaster_bridge.py").resolve()),
                "--endpoint",
                self.endpoint,
                "--worker-profile",
                self.profile,
    ...
    def _pipe_stderr(self) -> None:
        ...
        def run() -> None:
            ...
            for line in proc.stderr:
                text = line.rstrip()
                if text:
                    print(f"[kmaster-bridge-worker:{self.key}] {text}", file=sys.stderr, flush=True)

        threading.Thread(target=run, daemon=True, name=f"kmaster-bridge-worker-stderr-{self.key}").start()
    ...
    def _wait_ready(self) -> None:
        ...
        threading.Thread(target=read_stdout, daemon=True, name=f"kmaster-bridge-worker-stdout-{self.key}").start()
    ...
    def stop(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                print(f"[kmaster-bridge-worker:{self.key}] graceful shutdown failed: {exc}", file=sys.stderr, flush=True)
            proc.terminate()
            try:
                proc.wait(timeout=3)
...
def _worker_endpoint(key: str, namespace: str | None = None) -> str:
    ...
    if use_tcp:
        port_base = int(os.environ.get("KMASTER_BRIDGE_WORKER_PORT_BASE", "16880"))
        port_offset = int(safe[:4], 16) % 1000
        port = port_base + port_offset
        if os.name == "nt" and port >= 49152:
            port = 16880 + port_offset
        return f"tcp://127.0.0.1:{port}"
    root = Path(tempfile.gettempdir()) / "kmaster-agent-bridge-workers"
...
def _kill_windows_endpoint_occupants(endpoint: str) -> None:
    """Kill stale processes occupying the target port.

    Controlled by KMASTER_BRIDGE_KILL_PORT_OCCUPANT (default 0 = disabled).
    When enabled, taskkills any LISTENING process on the target TCP port.
    """
    if os.name != "nt":
        return
    enabled = os.environ.get("KMASTER_BRIDGE_KILL_PORT_OCCUPANT", "0").strip()
    ...
    for pid in _windows_listening_pids_on_port(port):
        try:
            print(
                f"[kmaster-bridge] killing stale process tree pid={pid} port={port}",
                file=sys.stderr,
                flush=True,
            )
            subprocess.run(
            ...
            print(
                f"[kmaster-bridge] failed to kill stale process pid={pid}: {exc}",
                file=sys.stderr,
                flush=True,
            )
...
def _read_json_request(conn: socket.socket) -> dict[str, Any]:
    """Read one complete NDJSON line from a connection using LineReader.

    This replaces the old recv(65536)-then-take-first-line approach which
    could truncate messages > 64KB or lose data on sticky packets.
    """
    reader = LineReader()
    while True:
    ...
    if reader.pending() > 0:
        # connection closed with partial data
        raise RuntimeError("connection closed before complete JSON line")
