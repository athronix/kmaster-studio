...
# ── M2: platform-specific file locking ──
if os.name == "nt":
    import msvcrt as _msvcrt

    def _acquire_file_lock(fd: int, timeout: float = 3.0) -> bool:
        """Acquire exclusive file lock on Windows (msvcrt.locking). Returns True on success, False on timeout."""
    ...
    def _release_file_lock(fd: int) -> None:
        """Release exclusive file lock on Windows."""
        try:
    ...
    def _acquire_file_lock(fd: int, timeout: float = 3.0) -> bool:
        """Acquire exclusive file lock on POSIX (fcntl.flock). Returns True on success, False on timeout."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            try:
    ...
    def _release_file_lock(fd: int) -> None:
        """Release exclusive file lock on POSIX."""
        try:
...
def _backup_file(path: str) -> str | None:
    """Copy *path* to *path*.bak. Returns the backup path on success, None on failure (non-blocking)."""
    bak_path = f"{path}.bak"
    try:
...
def _atomic_write_with_lock(path: str, content: str | bytes) -> dict[str, Any]:
    """Atomic write with file lock + backup + rollback.

    1. Open file + acquire exclusive lock (3s timeout)
    2. Create .bak backup
    3. Write new content → fsync
    4. Release lock

    Returns {"ok": True} on success, {"ok": False, "error": ...} on failure.
    On write failure: restores from .bak, then returns error.
    """
    tmp_path = f"{path}.tmp"
    bak_path = f"{path}.bak"
    fd = -1
    ...
    try:
        ...
        if not _acquire_file_lock(fd, timeout=3.0):
            os.close(fd)
            return {"ok": False, "error": {"code": "MCP_CONFIG_LOCKED",
                     "message": "配置文件被占用，请稍后重试"}}
...
class BridgeServer:
    """Worker-side action dispatcher.

    Runs inside each worker subprocess (--worker-profile mode).
    One BridgeServer per profile.  Owns the AgentPool.
    """
    ...
    def _save_mcp_config(self, cfg, profile=None):
        ...
        # M2: serialize YAML then atomic-write with file lock + .bak + rollback
        try:
            content = yaml.dump(cfg, sort_keys=False, allow_unicode=True)
            result = _atomic_write_with_lock(str(config_path), content)
            if not result.get("ok"):
                raise RuntimeError(result.get("error", {}).get("message", "atomic write failed"))
        except Exception as e:
    ...
    def _mcp_reload(self, req: dict, profile: str, _servers, _lock, run_on_mcp_loop,
        ...
        if target and target not in mcp_configs:
            return {"error": f"server '{target}' not found in config", "ok": False}
        if target:
        ...
        try:
            ...
            while not self._stop.is_set():
                ...
                try:
                    ...
                    print(f"[kmaster-bridge] server loop error: {exc}", file=sys.stderr, flush=True)
                finally:
                    if conn is not None:
                        try:
                            conn.close()
