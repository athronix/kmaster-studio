...
DEFAULT_ENDPOINT = "tcp://127.0.0.1:16765" if os.name == "nt" else "ipc:///tmp/kmaster-agent-bridge.sock"
...
def _install_windows_hidden_subprocess_defaults() -> None:
    ...
    class HiddenPopen(original_popen):  # type: ignore
    ...
    subprocess.Popen = HiddenPopen  # type: ignore
    asyncio.create_subprocess_exec = hidden_create_subprocess_exec  # type: ignore
    asyncio.create_subprocess_shell = hidden_create_subprocess_shell  # type: ignore
    subprocess._hermes_hidden_defaults_installed = True  # type: ignore
...
def _start_parent_process_watchdog(
    ...
    def run() -> None:
        while not stop_event.wait(interval):
            if _process_exists(parent_pid):
                continue
            print(
                f"[kmaster-bridge] parent pid {parent_pid} exited; stopping {label}",
                file=sys.stderr,
                flush=True,
            )
            stop_event.set()
    ...
    threading.Thread(target=run, daemon=True, name=f"kmaster-bridge-parent-watchdog-{label}").start()
...
def _bridge_log(event: str, payload: dict[str, Any]) -> None:
    try:
        ...
        print(
            "[kmaster_bridge] " + json.dumps(_sanitize_surrogates(body), ensure_ascii=False, default=_json_default),
            file=sys.stderr,
            flush=True,
        )
    except Exception:
        print(f"[kmaster_bridge] {event}", file=sys.stderr, flush=True)
...
def _refresh_terminal_env() -> None:
    ...
    try:
        ...
        print(
            f"[kmaster-bridge] Failed to refresh terminal env from {config_path}",
            file=sys.stderr,
            flush=True,
        )
...
def _discover_bridge_mcp_tools() -> list[str]:
    ...
    try:
        ...
        print(
            f"[kmaster_bridge] MCP tool discovery failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return []
