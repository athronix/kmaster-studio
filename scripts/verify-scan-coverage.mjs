...
import { EMOJI_RE, audit } from './uiux-audit.mjs';
...
const EXEMPT = [
  ...
  {
    id: 'E4-sentence-connector',
    reason:
      '→ 在句中作流程/路径连接符，删掉会改变语义（判 B 豁免），' +
      '但 EMOJI_RE 无法在字符级区分语境，故在此做位点级回退。' +
      '仅 SettingsDrawer.vue:77 的 → 是按钮尾随导航箭头（判 A）。',
    match: (site, ctx, key) => E4_SITES.has(key),
  },
];
...
// ══════════════════ E4 位点级豁免：从 tested 集扣除 B 类 sentence-connector ══════════════════
// E4 与 E1–E3 不同：E1–E3 处理 oracle 有、audit 没有的位点（oracle 多抓），
// E4 处理 audit 有、但按 B 类标准不应计为违规的位点（audit 多抓）。
// 机制：从 tested 集扣除 → 该位点落入 onlyOracle → EXEMPT 匹配 E4 → 显示为已豁免。
const E4_SITES = new Set([
  'packages/client/src/views/MemoryView.vue:202',
  'packages/client/src/components/dialog/NewTaskDialog.vue:211',
]);
for (const s of E4_SITES) tested.delete(s);
...
for (const k of onlyOracle) {
  ...
  const hit = EXEMPT.find((e) => ctx && e.match(site, ctx, k));
