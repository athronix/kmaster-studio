...
2. **emoji 扫描范围扩容**（v1/审计盲区）：emoji 不止在 `.vue`，`constants/layout.ts` + `locales/*.ts` 另有 **47 处**在 `.ts` 数据/i18n 层且真实渲染（已排除 JSDoc 注释与 `*.test.ts`，详见 §12.6 计数口径）。见 §3.2、§12。
...
| **C-3** | 功能性 emoji（`.vue` 模板区） | **116 ⚠️ 已作废** | **0** | 注释内 emoji、`→` 等排印符号；**116 口径不可复现（既非完整类 147 也非旧类 124，属第三套口径），已作废，等脚本权威值** |
| **C-4** | 功能性 emoji（`.ts` 数据/i18n 层） | **62** ⚠️ | **0** | 原旧字符类测 47（漏 `2300–23FF`/`25A0–25FF`/`2190–21FF` 三区间），扩类后真值 = 62（含 ⏰/↗/○◐⏳◉⌨️ 等 15 处类 A 图标），排除 JSDoc 与 `*.test.ts`（见 §12.6）；待脚本权威值校准 |
...
> ⚠️ **C-3 / C-4 的实测值远高于审计基线的「~45 功能性 emoji」**。差异来源见 §12.6 计数口径说明——这是本轮最重要的范围修正。
...
**C-3 `.vue` 功能性 emoji**（权威以脚本 `functionalEmoji` 中 `.vue` 部分 = 0 为准）
...
grep -rnP '[\x{1F300}-\x{1FAFF}\x{2300}-\x{23FF}\x{25A0}-\x{25FF}\x{2600}-\x{27BF}\x{2B00}-\x{2BFF}\x{2190}-\x{21FF}\x{FF0B}]' --include=*.vue . \
  ...
  | wc -l
```
**辅助抽查**：以下 grep 输出仅供人工核对，**不计为验收数字**（C-3 口径不可复现，已作废，等脚本权威值）。

**C-4 `.ts` 功能性 emoji**（权威以脚本 `functionalEmoji` 中 `.ts` 部分 = 0 为准；辅助标注同上 ⚠️）
```bash
...
grep -rnP '[\x{1F300}-\x{1FAFF}\x{2300}-\x{23FF}\x{25A0}-\x{25FF}\x{2600}-\x{27BF}\x{2B00}-\x{2BFF}\x{2190}-\x{21FF}\x{FF0B}]' --include=*.ts . \
  ...
  | wc -l
```
**辅助抽查**：以下 grep 输出仅供人工核对，**不计为验收数字**（C-4 为旧字符类产物，扩类后应为 62，等脚本权威值）。
...
| E-2 | 注释内 emoji / 排印箭头 `→` | 全局 | 不参与渲染，仅文档说明用途 | 长期 |
...
| 口径 | 数值 | 说明 |
|---|---|---|
| `.vue` 象形 emoji 原始出现次数 | 149 | 含注释 |
| `.vue` 排除注释行后 | **116 ⚠️ 已作废** | ← **C-3 基准（口径不可复现，等脚本权威值）** |
| `.vue` 箭头类符号（`←→↑↓↻` 等） | 102 | 绝大多数在 JSDoc 注释中，**不计入** |
| `.ts` 象形 emoji | **62 ⚠️** | ← **C-4 基准（旧字符类测 47，扩类后真值 62，待脚本权威值校准）** | 旧字符类（排除 JSDoc 与 `*.test.ts`）逐文件 47：`constants/layout.ts` 11 + `locales/*` 20 + `types/agent.ts` 8 + `composables/*List.ts` 7 + `stores/agentRoles.ts` 1；扩类补 `2300–23FF`/`25A0–25FF`/`2190–21FF` 三区间漏检 15 处（`layout.ts:130` ⏰、`en.ts`/`zh-CN.ts` 16–19 的 `▸↪⏹▸`、`agent.ts` 44–53 的 `○◐↗⏳◉⌨️`），真值 = 62。完整类重扫或更高（主理人全项目实测 213，见下方盲点；待架构师重扫确认） |
| **审计基线所称「功能性 emoji」** | ~45 | **口径偏小**：仅统计了 `.vue` 模板区的部分典型项，未含 `.ts` 数据层与 i18n 层 |

**结论**：整改面**临时规划基线冻结为 163 处**（Y5，主理人裁定，待脚本权威值一次性校准）。其构成「116 + 47」已随口径复核失效 —— **C-3（.vue 116）口径不可复现，已作废**；**C-4（.ts 47）为旧字符类产物，扩类后真值 = 62**（见上表）。故 163 不再由「116 + 47」支撑，而是作为窄类+位点口径的冻结下界沿用；主理人全项目实测完整类 = 213（纯漏检 +50 位点），权威数字待架构师扩类重扫后一次性修正。
...
- **（纪律四·单一计数口径）** 全文档任何量化结论只能来自 `scripts/uiux-audit.mjs`。禁止在文档中出现第二套可产生数字的检测方法（手写 grep 区间、目测清单、印象估计）。辅助命令必须显著标注「不得作为验收依据」。**溯源**：本轮发现文档 grep（147）与脚本（109）差 38（**.vue 范围**），其中 27 来自脚本缺陷、3 来自类 B 豁免、8 来自注释粒度 —— 两套口径并存导致每次对数都要重新解释一遍差值，是纪律一/二/三之后的第四类系统性内耗。（注：§12.2 C-3/C-4 的「当前」值 116/47 为早期测量，待脚本权威数字下发后一并校准；本溯源 147 为主理人本轮复跑文档 grep 的实测，用于证明 grep/脚本口径差。）
...
> ⚠️ **字符类盲点（主理人全项目实测确认，本轮最有价值发现）**：本节与旧版 §12.2 采用的 Unicode 字符类（仅 `1F300–1FAFF` / `2600–27BF` / `2B00–2BFF` / `FF0B`）**漏检了 `2300–23FF`（⏰ U+23F0、⏳ U+23F3、⌨️ U+2328）与 `25A0–25FF`（◉ U+25C9、◐ U+25D0、○ U+25CB）等符号块**。证据：`constants/layout.ts:130` 的 `⏰` 与 `types/agent.ts` 的 `AGENT_STATUS_ICONS`（`○◐⏳◉⌨️↗`）均为渲染链上真实 emoji，却被旧类漏掉。因此 **62（§12.2 C-4 表，扩类后真值；原旧类 47 已作废）与 163 是按窄类+位点口径的临时下界**，完整类真实值 ≈ 213（主理人全项目实测：窄类 163 / 完整类 213 / 纯漏检 +50 位点；漏检项均为核心交互控件图标 `✕` `⛶` `☰` `↻` `✎` `▾▸ ▲▼` 等，涉及 `PageHeader` / `RightPanel` / `ThoughtBlock` / `ToolCallCard`，非边角料）。权威数字待架构师扩类重扫后一次性修正。架构师新增的 `dataLayerEmoji` 指标**务必采用完整字符类**，否则会重复漏检数据层 emoji —— 这正是本次审计最初把数据层报成 0 的根因。
...
> 批次定义：**B1 首批**＝渲染缺陷（幽灵 Token＋状态色＋outline）＋Y3；**B2 数据/i18n 层**＝47 处 `.ts` emoji（含 Y4 最小改动）；**B3 模板层**＝116 处 `.vue` emoji（含 Y7）；**B4 像素尺寸**＝773px 硬编码尺寸；**B5 末批**＝theme.ts 双写→JS 调色板常量模块（Y2）。
...
| **Y5** | **整改面 163 处 ≫ 审计估算 45 处** | **已决策：分 5 批，数据层先于模板层** | 先清数据层（B2，47 处）再清模板层（B3，116 处），像素尺寸（B4，773px）单列，避免一次性大改引发回归 | **B1→B2→B3→B4→B5** |
